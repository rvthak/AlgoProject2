...
#define DELTA 1.6
#define GRID_AMOUNT 3

//------------------------------------------------------------------------------------------------------------------

Centroid *exact_centroid(Vector *v, CentroidArray *cent, double *d, VecDist distfun);
...
void report_results(std::string filename, string algorithm, bool complete_flag, bool silhouette_flag, CentroidArray* all_centroids);
...
int main(int argc, char *argv[]){
	...
	args.load_defaults(); args.ensure_compatability();
	args.print();
	...
	while( cent.changed() ){
		...
		if( args.update == "Vector" && args.assignment == "Classic" ) {
			algorithm = "AClassicUVector";
			Classic_assignment(&ass_vecs, &cent, &Vector::l2);
		}
		else if( args.update == "Frechet" && args.assignment == "Classic" ) {
			algorithm = "AClassicUFrechet";
			Classic_assignment(&ass_vecs, &cent, &Vector::discrete_frechet_distance);
		}

		else if( args.update == "Vector" && args.assignment == "LSH" ) {
			algorithm = "ALSHUVector";
		...
		if( args.update == "Vector" ){
			ass_vecs.update_centroids(&cent);
		} else{
			ass_vecs.update_curve_centroids(&cent);
		}
	...
	report_results(args.output_file, algorithm, args.complete, args.silhouette, &cent);
	std::cout << "\033[36;1m (T) \033[33;1mCompleted in: \033[0m" << t.toc() << " \033[33;1msec\033[0m" << std::endl;

	//------------------------------------------------------------------------------------------------------------------

	print_footer();
	if( args.update == "Vector"  && args.assignment == "LSH"       ){ delete lsh; }
	if( args.update == "Vector"  && args.assignment == "Hypercube" ){ delete cube; }
	if( args.update == "Frechet" && args.assignment == "LSH"       ){ delete grid; }
	return 0;
}
...
void Lsh_assignment(AssignmentArray *ass_vecs, MultiHash *lsh, CentroidArray *cent, VecDist distfun){
	...
	for(unsigned i=0; i<(cent->size); i++){
		...
		reverse_range_lsh_assignment(ass_vecs, cent, i, lsh , distfun);
	}
...
void report_results(std::string filename, string algorithm, bool complete_flag, bool silhouette_flag, CentroidArray* all_centroids){
	...
	if( silhouette_flag == true ){
		Timer t;
		std::cout << "\033[36;1m (I)\033[33;1m Calculating Silhouette... " << std::endl;  t.tic();
		vector<double> silhouette_results = get_sihlouette(all_centroids);
